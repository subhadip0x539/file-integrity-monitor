        ...
        if os.path.isfile(file):
            ...
            if data['status'] == 3:
                if compare_db_kin(data, alertlog):
                    ...
                    if analytics.objects().only('risk').first().risk == 0:
                        Thread(target=notify, args=(users, data, alertlog, analytics, baseline, baseline_bak, False)).start()
                        
                        
                    if analytics.objects().only('risk').first().risk == 1:
                        if match(data['file']):
                    ...
                    if analytics.objects().only('risk').first().risk == 2:
                        Thread(target=notify, args=(users, data, alertlog, analytics, baseline, baseline_bak, alert)).start()
                        if auto_enc:
                            zip(data['file_id'], "Encrypt", baseline, baseline_bak, analytics, keys)
