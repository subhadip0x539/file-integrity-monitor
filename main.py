...
from odd_jobs import compare_db, compare_db_gin, drop_collection, set_analyticsTozero, backup_baseline
...
def is_scanning(f):
    ...
    def decorator(*args, **kwargs):
        ...
        if req['cron'] and SETTINGS['wait_scan']:
...
def post_verifyrootlogin():
    if 'sess_id' in session:
        if users.objects(id=session['sess_id']).only('status').first().status == 1 and users.objects(id=session['sess_id']).only('role').first().role == 'root':
            return jsonify({"ack": "authorized"})
        elif users.objects(id=session['sess_id']).only('role').first().role == 'user':
            return jsonify({"error": "Unauthorized"}), 401
    else:
        return jsonify({"error": "Unauthorized"}), 401
...
def post_signup(req):
    ...
    log.write("{id}\t{role}\t{action}\t{time}\t{email}\n".format(
            id=users.objects(email=email).only('id').first().id, 
            email=email, 
            role="user", 
            action="signup", 
...
def post_authsignup():
    ...
    if users.objects(id=_id).only('role').first().role == 'root' and users.objects(id=_id).only('status').first().status == 1:
...
def post_baseline():
    ...
    backup_baseline(baseline, baseline_bak)
    SETTINGS["wait"] = False
...
def post_verify():
    ...
    if SETTINGS['cron']:
        SETTINGS['wait_scan'] = True
        start_cron()
    else:
        SETTINGS['wait_scan'] = False
        stop_cron()
...
def get_syslog():
    ...
    for doc in syslog.objects():
        ...
        for line in doc['logs']:
            sys_log.write("{}\t{}\t{}\t{}\t{}\t{}\n".format(
                line['file_id'], 
                line['hash'],
                datetime.fromtimestamp(line['createdate']), 
                datetime.fromtimestamp(line['modifydate']), 
                line['status'],
                line['file']))
        sys_log.write("\n\n\n\n")
    ...
    sys_log = open("syslog.txt", "rb")

    return send_file('syslog.txt')
